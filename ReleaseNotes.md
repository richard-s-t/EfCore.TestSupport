...
- Introduced a new package `EfCore.TestSupport.Core` without any xUnit-specific helpers. The main `EfCore.TestSupport` package now depends on this new package and includes the xUnit-specific helpers. This allows users who do not want to use xUnit to still benefit from the core features of the library without the additional dependencies.
